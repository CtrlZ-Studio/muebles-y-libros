...
const ContactSection = ({ contactData }) => {
  return (
    <article className="relative h-screen text-white scroll-snap-start bg-accent-dark">
      <header className="absolute top-0 left-0 right-0 tw-header bg-white border-accent-dark text-accent-dark">
        {WEB_NAME} // {contactData.heading}
      </header>

      <section className="flex flex-col items-center justify-center h-full tw-px tw-pt space-y-6 lg:space-y-10 2xl:space-y-12 text-base lg:text-xl 2xl:text-2xl">
        <div className="w-full lg:w-1/3">
          <IconLogo />
        </div>

        <div className="flex space-x-12 lg:space-x-20 3xl:space-x-24">
          <a
